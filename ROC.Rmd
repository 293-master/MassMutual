...
# Actual scenario: The p_hat are fitted from an actual logistic regression model
# (I didn't give the model's details)
values <- read_csv("https://rudeboybert.github.io/SDS293/static/methods/ROC/example.csv")

# Alternative scenario 1.a): Randomly scramble the fitted p_hat. This is like guessing at random.
# Uncomment this:
# values <- values %>%
#   mutate(p_hat = sample(p_hat))

# Alternative scenario 1.b): Assign random p-hats between 0 and 1. This is also like guessing at random.
# Uncomment this:
# values <- values %>%
#   mutate(p_hat = runif(n(), min = 0, max = 1))

# Alternative scenario 2: Make reverse predictions.
# Uncomment this:
# values <- values %>%
#   mutate(p_hat = 1-p_hat)

# Alternative scenario 3: Make "perfect" predictions.
# Uncomment this:
# values <- values %>%
#   mutate(p_hat = y)

# Alternative scenario 4: Make "perfect wrong" predictions.
# Uncomment this:
# values <- values %>%
#   mutate(p_hat = 1-y)
```

In our data set of `r nrow(values)` individuals, 24116 have $y$ = 1 and 35827 $y$ = 0. The $\widehat{p}$ values here are obtained from a model, like logistic regression.

Let's take a look at a random sample of 10 rows of our data that we'll
